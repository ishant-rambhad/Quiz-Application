...
class display:
    def __init__(self,root) :
        ...
        #frame2 
        login   =Button(self.root,text="LOGIN",font=("arial",15,"bold"),bd=5,width=20,cursor="hand2",bg="sky blue",activebackground="gray",command=self.login_fun)
        login.grid(row=0,column=1,padx=60,pady=160)
       
        register=Button(self.root,text="REGISTER",font=("arial",15,"bold"),bd=5,width=20,cursor="hand2",bg="sky blue",activebackground="gray",command=self.register_fun)
        register.grid(row=0,column=2,padx=60,pady=160)
       
        create  =Button(self.root,text="CREATE NEW QUIZ",font=("arial",15,"bold"),bd=5,width=20,cursor="hand2",bg="sky blue",activebackground="gray",command=self.create_fun)
        create.grid(row=0,column=3,padx=60,pady=160)
            
        exit  =Button(self.root,text="EXIT",font=("arial",15,"bold"),bd=5,width=20,cursor="hand2",bg="sky blue",activebackground="gray",command=self.exit_fun)
        exit.grid(row=0,column=6,padx=60,pady=160)
               
        self.frame2=Frame(self.root,height=500,width=700,background="white").place(x=800,y=250)

        stdtittle = Label(self.frame2, text="Student section", font=("times new roman",28, "bold"),bg="white")
        stdtittle.place(x=1000,y=250)
        
        #id 
        id=Label(self.frame2,text="Topic:",font=("times new roman",16,"bold"),bg="white")
        id.place(x=950,y=350)
        
        id_Entry=ttk.Entry(self.frame2,textvariable=self.id,font=("arial",15),width=22)
        id_Entry.place(x=1050,y=350)

      
      #password
        pwd=Label(self.frame2,text="Password:",font=("times new roman",16,"bold"),bg="white")
        pwd.place(x=950,y=400)
        
        pwd_Entry=ttk.Entry(self.frame2,font=("arial",15),textvariable=self.pwd,width=22,show="*")
        pwd_Entry.place(x=1050,y=400)

      #username
        user_name=Label(self.frame2,text="Username:",font=("times new roman",16,"bold"),bg="white")
        user_name.place(x=950,y=450)
        
        userName_Entry=ttk.Entry(self.frame2,textvariable=self.username,font=("arial",15),width=22)
        userName_Entry.place(x=1050,y=450)

      #email
        email=Label(self.frame2,text="Email:",font=("times new roman",16,"bold"),bg="white")
        email.place(x=950,y=500)
        
        email_Entry=ttk.Entry(self.frame2,font=("arial",15),textvariable=self.email,width=22)
        email_Entry.place(x=1050,y=500)
