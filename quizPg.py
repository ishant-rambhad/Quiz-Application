...
class display:
    ...
    def __init__(self,root,username="") :
      ...
      # Create a cursor object to interact with the database
        ...
        cursor.execute("select ID from "+username)
        self.UserName_rows=cursor.fetchall()
        self.out_of=len(self.UserName_rows)

        query = "SELECT * FROM "+username+" WHERE ID = %s"
        ...
        que_db.place(x=300,y=100,height=90,width=1080)
    ...
    def ans(self):
        ...
        query = "SELECT Answer FROM "+self.username+" WHERE ID = %s"
        cursor.execute(query, (self.question_num,))
